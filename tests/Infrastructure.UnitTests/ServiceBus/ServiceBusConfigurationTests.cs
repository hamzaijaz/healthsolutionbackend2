...
namespace CapitalRaising.RightsIssues.Service.Infrastructure.UnitTests.ServiceBus
{
    public class ServiceBusConfigurationTests
    {
        ...
        public void EnsureOtherConnectionStringsAreLoaded_WhenMultipleSettingsExist()
        {
            // arrange
            var configSettings = new List<KeyValuePair<string, string>>();
            configSettings.Add(new KeyValuePair<string, string>("ServiceBusConnection:Queue1", "SomeConnectionStringForQueue"));
            configSettings.Add(new KeyValuePair<string, string>("ServiceBusConnection:Topic1", "SomeConnectionStringForTopic"));
            IConfiguration config = SetupConfiguration(configSettings);
