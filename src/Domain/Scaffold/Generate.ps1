...
Get-ChildItem -Recurse | ForEach { (Get-Content $_.PSPath | ForEach {$_ -creplace "EFCoreScaffold", "$($ApplicationName).Service.Domain.Entities"}) | Set-Content $_.PSPath }
...
$dbContext = $dbContext -creplace "EFCoreScaffold", "$($ApplicationName).Service.Infrastructure.Persistence"
$dbContext = $dbContext -creplace "using System;", "using System;`r`nusing $($ApplicationName).Service.Domain.Entities;"
