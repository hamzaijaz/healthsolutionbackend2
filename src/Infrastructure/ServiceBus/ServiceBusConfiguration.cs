...
namespace MyHealthSolution.Service.Infrastructure.ServiceBus
{
    ...
    public class ServiceBusConfiguration : IServiceBusConfiguration
    {
        ...
        public ServiceBusConfiguration(IConfiguration configuration)
        {
            ...
            DefaultConnectionString = _configuration.GetValue<string>("ServiceBusConnection");
        }
            ...
            get
            {
                if (_OtherConnectionStrings == null)
                {
                    ...
                    var otherConnections = from configItem in Configurations(_configuration)
                                           where configItem.Key.StartsWith("ServiceBusConnection:")
                                           select new { Key = configItem.Key.Remove(0, "ServiceBusConnection:".Length), Value = configItem.Value };
